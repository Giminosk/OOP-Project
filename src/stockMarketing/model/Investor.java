 ...
 * @author Giminosk
 */
public class Investor extends PrivateInvestor implements Runnable {
    private StringProperty id;
    private DoubleProperty investmentBudget;
    
    private StockMarket memberOfStockMarket;
    ...
    public Investor(String firstName, String lastName, String id, Double budget, StockMarket memberOfStockMarket) {
        super(firstName, lastName);
        this.id = new SimpleStringProperty(id);
        this.investmentBudget = new SimpleDoubleProperty(budget);
        this.shares = new HashMap<>();
        this.commodities = new HashMap<>();
        this.memberOfStockMarket = memberOfStockMarket;
    }
    ...
    public void buyShare() throws InterruptedException{
        ...
        synchronized(this.getStockExchangeBelonging()){
            List<Company> companies = this.memberOfStockMarket.getCompanies();
    ...
    public void buyCommodity() throws InterruptedException{
        ...
        synchronized(this.getCommodityMarketBelonging()){
            ...
            if (commodity.getCommoditiesForSale() > 0 && this.getBudget() > 0) {
                ...
                int commoditiesToBuy = (int) ((int) this.getBudget() / commodity.getCurrentPrice());

                if (commodity.getCommoditiesForSale() - commoditiesToBuy >= 0) {
                    double value = commoditiesToBuy * commodity.getCurrentPrice() ;
                    this.getCommodities().put(commodity, commoditiesToBuy);                    
                    this.setBudget(this.getBudget() - value);
                    
                    commodity.getTransactionList().add(new Transaction(commodity.getCurrentPrice(), commoditiesToBuy));
                    ...
                    double value = availableCommodities * commodity.getCurrentPrice();
                    this.getCommodities().put(commodity, availableCommodities);                                       
                    this.setBudget( this.getBudget() - value);

                    commodity.getTransactionList().add(new Transaction(commodity.getCurrentPrice(), availableCommodities));
                    commodity.calculateCurrentPrice();
        ...
        synchronized(this.getCommodityMarketBelonging()){
            if (!this.getCommodities().isEmpty()){
                ...
                double value = investorCommodities * commodity.getCurrentPrice();
                double income = value * (1 - this.getCommodityMarketBelonging().getFee()/100);

                commodity.getTransactionList().add(new Transaction(commodity.getCurrentPrice(), investorCommodities));
    ...
    public final double getBudget() {
        return investmentBudget.get();
    }

    public final void setBudget(double value) {
        investmentBudget.set(value);
    }
    ...
    public final StockMarket getStockExchangeBelonging () {
        return memberOfStockMarket;
    }
    
    public final void setStockExchangeBelonging(StockMarket stockMarket) {
        memberOfStockMarket = stockMarket;
    }
